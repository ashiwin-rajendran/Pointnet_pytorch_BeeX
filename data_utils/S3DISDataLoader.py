...
class S3DISDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(100):
            ...
            if point_idxs.size > 32:
                break
        else:
            # Fallback: use all points if no valid block found
...
if __name__ == "__main__":
    ...
    train_loader = torch.utils.data.DataLoader(
        point_data, batch_size=16, shuffle=True, num_workers=16, pin_memory=True, worker_init_fn=worker_init_fn
    )
    for idx in range(4):
        end = time.time()
